...
public class ChatMessageDataBase {
    ...
    private ChatMessageDataBase() {
        helper = new DbHelper(SampleApplicationLike.getAppContext(), "chat.db");
    ...
    public boolean isShowTime(String chatJid, long msgTime) {
        ...
        String sql = "select max(" + TableField._FIELD_SEND_TIME + ") from " + TableField
                ._TABLE_CHAT
                + " where " + TableField._FIELD_CHAT_JID + "=? and " + TableField
                ._FIELD_SHOW_TIME + "=1";
    ...
    public void clearUnReadByJid(String chatJid) {
        ...
        db.update(TableField._TABLE_CHAT, values, TableField._FIELD_CHAT_JID + "=?", new
                String[]{chatJid});
    }

    public ArrayList<ChatMessageVo> getChatMessageListEvent() {
        ArrayList<ChatMessageVo> list = new ArrayList<>();
        String sql = "select * ,sum(_unread) total_unread from chat group by chatjid order by " +
                "sendtime desc";
    ...
    private ChatMessageVo getChatMessageVoByCursor(Cursor cursor) {
        ...
        msg.setShowTime(cursor.getInt(cursor.getColumnIndex(TableField._FIELD_SHOW_TIME)) == 1 ?
                true : false);
        msg.setMe(cursor.getInt(cursor.getColumnIndex(TableField._FIELD_IS_ME)) == 1 ? true :
                false);
